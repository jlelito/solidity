...
function App() {
  ...
  async function updateShares() {
    const shares = await contract.methods.shares(accounts[0]).call();
    setShares(shares);
  }
  ...
  async function redeemShares(e) {
    e.preventDefault();
    const amount = e.target.elements[0].value;
    await contract.methods.redeemShare(amount).send({from: accounts[0]});
    await updateShares();
  };
  ...
  async function transferShares(e) {
    e.preventDefault();
    const amount = e.target.elements[0].value;
    const toAddress = e.target.elements[1].value;
    await contract.methods.transferShare(amount, toAddress).send({from: accounts[0]});
    await updateShares();
  };
  ...
  return (
    <div className="container">
     ...
     <p>Current Account: <b>{accounts[0]}</b></p>
      <p>Shares: {shares}</p>
