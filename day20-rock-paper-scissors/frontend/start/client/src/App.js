...
function App() {
  ...
  async function createGame(e) {
    ...
    await contract.methods.createGame(participant).send({from: accounts[0],  value: bet});
    await updateGame();
  };
  ...
  async function commitMove(e) {
    ...
    //List of all options of select, selectedIndex is the current selected value
    const choiceId = select.options[select.selectedIndex].value;
    const salt = Math.floor(Math.random() * 1000);
    await contract.methods.commitMove(game.id, choiceId, salt).send({from: accounts[0]});
    setMove({id: choiceId, salt});
    await updateGame();
